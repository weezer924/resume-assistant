...
- Week 3 开始前，FactDraft 包含 claim、evidence_quote、source_sequence。抽取后只返回草稿并保存 Run；确认接口才插入 facts。
...
| 确认 | pending | 同一条记录变 confirmed | 按候选 ID 更新，记录确认时间；不重复校验来源与引用 |
...
候选持久化、确认和编辑已进入实现；继续保留这些成果，不重新开始。
当前执行顺序：
...
confirmed 证据。多条候选的空结果和失败保存策略在该切片开始前明确，
不在本次文档调整中擅自决定。
...
Jack 决定将简单 HTML MVP 提前，以实际页面观察数据流。页面位于
`frontend/`，由 FastAPI 同源提供；当前串联导入、选择持久化片段、抽取、确认、
编辑和刷新恢复。拒绝、职位分析仍是 Week 3 后续任务。
仅当前文档 ID 保存在浏览器 sessionStorage；刷新后从 SQLite 读取原文和候选，
不将履历正文或模型输出放入浏览器持久存储。此 MVP 暂不提供历史文档列表。
旧数据库迁移尚未完成；使用独立的新数据库体验 MVP，不删除或自动修改旧数据。
