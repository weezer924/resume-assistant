# Evidence-Backed Resume Assistant — Product and Technical Specification

Status: **Draft for review**

Read the specification in this order:

1. [Product foundations](./spec/01-product-foundations.md) — Sections 1–5
2. [Data model](./spec/02-data-model.md) — Section 6
3. [Domain invariants and fixed pipeline](./spec/03-domain-invariants-pipeline.md) — Sections 7–8
4. [AI and evaluation](./spec/04-ai-and-evaluation.md) — Sections 9–13
5. [Application boundary and technical shape](./spec/05-application-boundary-and-technical-shape.md) — Sections 14–16
6. [Technology stack](./spec/06-technology-stack.md) — Section 17
7. [Repository layout](./spec/07-repository-layout.md) — Section 18
8. [Milestones, learning, and approval](./spec/08-milestones-acceptance-criteria.md) — Sections 19–24

Decision records live in [spec/adr/](./spec/adr/). For Week 3, also read the approved [field and lifecycle alignment](./spec/adr/week3-model-alignment.md).

The overall specification remains **Draft for review**. Implementation beyond explicitly requested exercises waits until the approval checklist in [Section 24](./spec/08-milestones-acceptance-criteria.md#24-specification-approval-checklist) is reviewed and the overall status becomes **Approved**. Approval of a scoped alignment does not imply approval of the entire specification.

## Local review UI

For the Week 3 HTML MVP, see [startup and workflow instructions](frontend/README.md).
